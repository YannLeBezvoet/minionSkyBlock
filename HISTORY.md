...
A real `minecraft:nether_portal` block was deliberately **not** used: that block is hardcoded by the game to teleport to the Nether/Overworld specifically, so using it here would fight vanilla's own portal logic instead of going to `minionskyblock:mining`. `particle minecraft:portal` is spawned in each walkway every tick for the shimmer effect, and the actual dimension travel is still a `teleport` command, same as the villager version. The obsidian frames themselves were first built with `fill` commands in `world/build_portal.mcfunction` / `world/mining_init.mcfunction`, then moved into the `minionskyblock:island` / `minionskyblock:miningspawn` structures (authored in-game with a structure block, same as the rest of each structure's terrain) since a decorative frame belongs with the rest of the terrain, not re-forced by command every load — both functions now only retroactively `kill` the old villager tags.
