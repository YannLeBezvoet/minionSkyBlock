...
## Prospector NPC replaced by a portal (2026-07-26)

The two Prospector villagers (overworld side at `5 66 0`, mining-side inside the arrival room at `4 21 2`) that gated entry/exit of the Mining dimension were replaced with freestanding obsidian doorways detected by player position instead of right-click, on explicit request ("un vrai portail" instead of a villager). Deleted: `world/prospector_clicked.mcfunction`, `world/prospector_clicked_return.mcfunction` (the interaction-consuming reward functions — no longer needed since detection is now a plain position check run directly from `player/on_tick.mcfunction`, not an advancement-style click reward). Renamed: `world/prospector_init.mcfunction` → `world/build_portal.mcfunction`, `world/prospector_teleport_out.mcfunction` → `world/portal_teleport_out.mcfunction`, `world/prospector_teleport_back.mcfunction` → `world/portal_teleport_back.mcfunction`. Both `build_portal.mcfunction` and `mining_init.mcfunction` retroactively `kill` the old `prospector`/`prospector_interaction`/`prospector_return`/`prospector_return_interaction` tags so worlds created before this change don't keep an orphaned villager pair around forever. New scoreboard `skyblock_portal_cd` (same per-player cooldown pattern as `skyblock_hive_cd`) prevents the position check from re-firing the instant a player arrives, mirroring vanilla's own nether portal cooldown — a discrete click never needed this, but a continuous "standing in the walkway" check does.

A real `minecraft:nether_portal` block was deliberately **not** used: that block is hardcoded by the game to teleport to the Nether/Overworld specifically, so using it here would fight vanilla's own portal logic instead of going to `minionskyblock:mining`. `particle minecraft:portal` is spawned at each teleport zone every tick for the shimmer effect, and the actual dimension travel is still a `teleport` command, same as the villager version.

The obsidian frame itself went through two iterations before being dropped entirely, both by explicit request: first built with `fill` commands directly in `world/build_portal.mcfunction` / `world/mining_init.mcfunction`; then, since a decorative frame arguably belongs with the rest of the terrain rather than being re-forced by command every load, moved into the `minionskyblock:island` / `minionskyblock:miningspawn` structures (authored in-game with a structure block, same as the rest of each structure's terrain). Both `.nbt` files were then reverted to their pre-frame blobs (`git checkout <commit before the merge that added the frame> -- data/minionskyblock/structure/island.nbt data/minionskyblock/structure/miningspawn.nbt`) in favor of **no physical frame at all** — just the particle marker, which is simpler and leaves nothing for a player to accidentally mine away. `world/build_portal.mcfunction` and `world/mining_init.mcfunction` now only retroactively `kill` the old villager tags; they place no blocks. The mining-side teleport zone was also moved from `Z=2` to `Z=4` to match the particle position, which happens to be the exact spot `world/portal_teleport_out.mcfunction` lands the player on arrival — accepted trade-off: standing there for more than the 2-second `skyblock_portal_cd` cooldown auto-returns the player to the overworld.
