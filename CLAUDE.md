...
data/
  ...
  minionskyblock/
    ...
    function/
      ...
      world/build_portal.mcfunction← retroactive-only: kills the old overworld Prospector villager + its interaction entity (replaced by a portal frame now baked into island.nbt — see HISTORY.md), called from load.mcfunction; self-reschedules 1t at a time via `execute unless loaded ... run schedule ...` until chunk (0,0) is actually loaded — see Known Gotchas' "Prospector NPC duplicated on some world loads"
...
| Portal (overworld side, entry into Mining dimension) | 5 66 0, walkway X=5 Y=66-68 Z=0-1 |
| Portal (Mining dimension side, inside the arrival room, return trip) | walkway X=4 Y=21-23 Z=2 (minionskyblock:mining) |
...
**Portal — the real, player-facing entry point** (replaces the earlier Prospector villager NPC, see HISTORY.md): a freestanding obsidian doorway — not a real `minecraft:nether_portal` block, since that block is hardcoded by the game to teleport to the Nether/Overworld specifically, so using it here would fight vanilla's own portal logic. Both frames are plain obsidian blocks baked into the pack's own structures, authored in-game with a structure block, same workflow as the rest of each structure's terrain: the entry doorway at `5 66 0` on the island's NPC platform (see Key coordinates) is part of `minionskyblock:island` (see "Initial island generation"), walkway `X=5 Y=66-68 Z=0-1`; the exit doorway at `4 21 2` inside the Mining dimension's arrival room is part of `minionskyblock:miningspawn` (see `world/build_arrival_room.mcfunction` below), walkway `X=4 Y=21-23 Z=2`. Neither function/structure enforces the frame's blocks after initial placement — a player mining the obsidian doesn't get it back, same as the rest of the island/arrival-room terrain, since the walk-through detection is purely coordinate-based and doesn't depend on the obsidian actually being there. `world/build_portal.mcfunction` and `world/mining_init.mcfunction` only retroactively `kill` the old `prospector`/`prospector_interaction`/`prospector_return`/`prospector_return_interaction` tags, since worlds created before this change still have that villager pair and nothing summons/kills it anymore — both still gated behind their own `unless loaded` self-reschedule (see the chunk-loading gotcha below), since `kill` needs the chunk loaded just like `fill`/`setblock` do.
  - **Detection**: `player/on_tick.mcfunction` checks `if entity @s[x=...,y=...,z=...,dx=...,dy=...,dz=...]` — a bounding-box selector test against `@s` itself — against each walkway's box every tick, gated on `if data entity @s {Dimension:"..."}` so an overworld player's box check can never accidentally match the mining-side coordinates or vice versa (same `Dimension` NBT pattern as the `@a[dimension=...]` gotcha above). No interaction entity needed at all: since this whole function already runs as/at the player via the `tick_loop` advancement reward, `@s` **is** the player, so it's a plain "am I standing here" check rather than a right-click. A per-player `skyblock_portal_cd` cooldown (mirrors `skyblock_hive_cd`'s pattern, decremented once per tick, set to `40` by `world/portal_teleport_out.mcfunction` / `_teleport_back.mcfunction`) blocks the box check from re-firing the instant the player arrives — same purpose as vanilla's own nether portal cooldown, and unlike the interaction-entity approach it's needed here because standing in a walkway is a continuous state, not a discrete click.
  - **Ambient portal particles**: `player/on_tick.mcfunction` also fires `particle minecraft:portal ...` at each walkway's center every tick a player is in the matching dimension — purely cosmetic, no gameplay effect, same `Dimension`-gated pattern as the detection check above.
