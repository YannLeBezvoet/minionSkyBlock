...
data/
  ...
  minionskyblock/
    ...
    function/
      ...
      world/prospector_init.mcfunction← kill-then-resummon of the overworld Prospector + its interaction entity, called from load.mcfunction; self-reschedules 1t at a time via `execute unless loaded ... run schedule ...` until chunk (0,0) is actually loaded — see Known Gotchas' "Prospector NPC duplicated on some world loads"
      world/mining_init.mcfunction← forceloads chunk (0,0) of minionskyblock:mining, self-reschedules 1t at a time via `execute unless loaded ... run schedule ...` (same pattern as prospector_init.mcfunction) until that chunk is actually loaded, then runs the arrival-room build guard and the kill-then-resummon of the prospector_return NPC pair — see Known Gotchas' "Mining arrival room rebuilt itself on reconnect"
      world/prospector_clicked.mcfunction← reward for the overworld Prospector's interaction entity, teleports the nearest player into the Mining dimension (see "Mining dimension")
      world/prospector_clicked_return.mcfunction← same, for the Prospector standing in the Mining dimension's arrival room, teleports back to the island
      world/prospector_teleport_out.mcfunction← actual teleport + actionbar into minionskyblock:mining
      world/prospector_teleport_back.mcfunction← actual teleport + actionbar back to minecraft:overworld
```
...
| Prospector NPC (overworld side, entry into Mining dimension) | 5 66 0 |
| Prospector NPC (Mining dimension side, inside the arrival room, return trip) | 4 21 2 (minionskyblock:mining) |
...
- `function/world/mining_tp.mcfunction` / `mining_return.mcfunction` — manual-only test commands (same convention as `debug.mcfunction`), kept for quick testing without walking to the NPC: `/function minionskyblock:world/mining_tp` (re)builds the arrival room and teleports in (`4 21 4`); `.../mining_return` comes back to `0 66 0` in the overworld.

**Prospector NPC — the real, player-facing entry point**: a `minecraft:villager` (`NoAI:1b`, `Invulnerable:1b`, tag `prospector`) stands at `5 66 0` on the island's NPC platform (see Key coordinates), paired with a `minecraft:interaction` entity (tag `prospector_interaction`) summoned on top of it — same two-entity pattern used by every other NPC this pack has ever had (see HISTORY.md). A second identical pair (tags `prospector_return` / `prospector_return_interaction`) stands at `4 21 2` inside the Mining dimension's arrival room, kill-then-resummoned by `world/mining_init.mcfunction` right after the arrival-room build guard, both gated behind that function's own `unless loaded 0 0 0` self-reschedule (see the chunk-loading gotcha below — an earlier version assumed the preceding `forceload add` alone was enough and rebuilt the room on some reconnects). The overworld pair goes through `world/prospector_init.mcfunction` instead of being kill/summoned inline in `load.mcfunction` — same self-reschedule pattern, see the chunk-loading gotcha below.
  - **Detection**: `player/on_tick.mcfunction` polls `if data entity @s interaction.player` on both interaction entities every tick (same mechanism as minion pickup's `attack.player` check, just the right-click variant instead of left-click) → `world/prospector_clicked[_return].mcfunction` consumes the interaction (`data remove entity @s interaction`, preventing it from re-firing next tick) and hands off to `world/prospector_teleport_out.mcfunction` / `_teleport_back.mcfunction` for the actual `teleport` + actionbar. Because each player's `on_tick` executes in whatever dimension that player is physically in, the entity-tag lookup is automatically dimension-scoped — no explicit dimension check needed (an overworld player's tick will never find the mining-side interaction entity, and vice versa).
  - **Click range**: `execute at @s as @p[distance=..3] run ...` (same as the old Prospector) — the nearest player within 3 blocks of the NPC's own position is the one teleported, so it can't be triggered by a distant player's interaction entity check coincidentally matching.
  - **Why this needed no new advancement**: unlike minion placement/pickup (which reacts to a player action on an item/entity the player owns), the Prospector is a persistent, always-present NPC — polling its interaction entity directly from the existing per-player tick loop is simpler and avoids one more `item_used_on_block`-style trigger with its own gotchas (see Known Gotchas).

**To test**: `./update.sh`, then start a **fresh** world (delete/recreate, or a new save) — a brand-new dimension needs a full world (re)open anyway (see the `/reload` gotcha above). Right-click the Prospector on the island to go in, right-click the one in the arrival room to come back.

**Verified in-game**: spawn rate/proximity of the 3 active structures (mineshaft, stronghold, trial chambers) with these denser `structure_overrides` values in a flat world, and the Prospector round trip both ways. Ancient City is currently disabled — see above.
