...
- `data/minionskyblock/dimension_type/mining.json` — **custom dimension_type** (`minionskyblock:mining`), `has_skylight: false` + `skybox: "none"` (no rendered sky), `has_ceiling: true`, cave ambience (`minecraft:ambient.cave` mood sound). `min_y: 0`, `height: 128`, `logical_height: 128`.
- `data/minionskyblock/dimension/mining.json` — `generator.type: minecraft:flat`, `settings.biome: minecraft:plains` (a real vanilla biome — gets normal ore-vein generation and mineshaft eligibility for free, no custom biome/tag needed), `layers`: 1 block `minecraft:bedrock` (Y=0, floor) + 126 blocks `minecraft:stone` (Y=1-126) + 1 block `minecraft:bedrock` (Y=127, ceiling), `structure_overrides: ["minecraft:mineshaft"]` (forces mineshaft generation on top of whatever the biome would naturally allow).
  - **Why this closes the ceiling-gap problem for free**: a flat generator doesn't shape terrain at all — no density functions, no caves/ravines/aquifers carving air anywhere. The only air in the dimension is what mineshaft structures or ore-vein features explicitly place, and both bedrock layers are **real placed blocks**, not a rule applied after the fact — nothing can "skip" placing them the way a density-function surface_rule could be undermined by cave carving. The entire class of bugs from the old noise-based version (surface_rule gaps, `fill_layer` ordering, cross-chunk structure edge cases) doesn't apply here because there's no terrain shaping step for anything to slip through.
  - **Why `minecraft:plains` and not a cave biome**: ore-vein placed features are attached per-biome in vanilla and plains carries the standard set (coal/iron/copper/gold/redstone/lapis/diamond). A cave biome would need the same custom-biome-plus-tag dance the old version used just to guarantee mineshaft eligibility; plains already qualifies natively.
- `function/world/build_mining_dimension_poc.mcfunction` — carves one small **safe arrival room** (floor Y=60, walls Y=61-64, ceiling Y=65, torches, at X0-8/Z0-8) inside the solid stone — the room's only job is to guarantee the player doesn't land inside solid rock on arrival; everywhere outside it is untouched stone to mine into. Called from `load.mcfunction` behind `execute in minionskyblock:mining unless block 1 61 1 minecraft:torch run ...`. Guard checks for a **torch**, not `minecraft:stone` — the flat generator's own layers already place stone at the guard coordinates, which would make a stone-based guard falsely think the room was already built.
- `function/world/mining_dimension_poc_tp.mcfunction` / `mining_dimension_poc_return.mcfunction` — manual-only test commands (same convention as `debug.mcfunction`): `/function minionskyblock:world/mining_dimension_poc_tp` (re)builds the arrival room and teleports in (`4 61 4`); `.../mining_dimension_poc_return` comes back to `0 66 0` in the overworld.
...
**Not yet verified in-game**: mineshaft spawn rate/proximity to the arrival room with `structure_overrides` in a flat world — flag it back if none show up after reasonable exploration.
