...
Consumable items that let a player retroactively change the biome of the 16×16 chunk they're standing in — e.g. to a `desert` for hotter mob-spawn rules and no snow. Not wired into a crafting recipe yet (deliberately deferred — see `function/biome/give_desert_stone.mcfunction` below); not documented in HowToPlay.md until it is, since it isn't reachable by normal play yet.

**Storage-driven, same extensibility pattern as Minions** (see "To add a new minion type" below): `minionskyblock:biome_changer` holds one entry per biome type, keyed by the type name (currently only `desert`), set in `load.mcfunction`:
...
| Field | Example | Usage |
| --- | --- | --- |
| `biome` | `"minecraft:desert"` | Full biome ID passed to `fillbiome` |
| `name` | `"Desert"` | Used in the actionbar confirmation message |
...
- `advancement/biome/change_desert.json` — trigger `minecraft:consume_item`, `predicates.minecraft:custom_data = {"biome_item": "desert"}` → reward `minionskyblock:biome/change_desert`
- `function/biome/change_desert.mcfunction` — 1-line wrapper: `function minionskyblock:biome/change with storage minionskyblock:biome_changer desert` (same 1-liner-per-type convention as `minion/place_<type>.mcfunction`)
...
- `function/biome/give_desert_stone.mcfunction` — manual-only test give (same convention as `debug.mcfunction`/`mining_tp`), not called by any tag/advancement: `/function minionskyblock:biome/give_desert_stone`. Base item `minecraft:sand` (thematic, no special vanilla behavior to fight), same consumable-food component trick as minion items (`minecraft:food`+`minecraft:consumable`, `can_always_eat:true` mandatory — see gotcha).
...
**Not done yet**: no crafting recipe (test via the manual give function above only), only one biome (`desert`) implemented — adding another follows the same steps as "To add a new minion type" below, minus the tiering (just one storage entry per biome, one advancement, one 1-line wrapper function; `change.mcfunction`/`apply.mcfunction` are already generic and don't need touching).
